...
class AggregatedEvent(Event):
    ...
    def __init__(self, events: List[Event], probability: float):
        self.type = None if not events else events[0].type  # will not be set correctly for nested Kleene closures
        ...
        self.min_timestamp = self.timestamp = None if not events else events[0].timestamp
        self.max_timestamp = None if not events else events[-1].timestamp
